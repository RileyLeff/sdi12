...
use {
    ...
    alloc::{string::{String, ToString}, vec::Vec}, // Keep Vec and ToString
};
...
pub fn parse_response(buffer: &[u8]) -> Result<Response, ResponseParseError> {
    ...
    let payload_without_crc = if payload_with_maybe_crc.len() >= 4 {
        ...
        if potential_crc_bytes[0] & 0xC0 == 0x40 && potential_crc_bytes[1] & 0xC0 == 0x40 && potential_crc_bytes[2] & 0xC0 == 0x40 {
            ...
            if calculated_crc == decoded_crc {
                ...
                data_part
            } else { return Err(ResponseParseError::CrcMismatch); }
        } else { payload_with_maybe_crc }
    } else { payload_with_maybe_crc };
    ...
    match remaining {
        ...
        b"" => {
            if crc_val.is_some() { Ok(Response::Aborted { address, crc: crc_val }) }
            else { Ok(Response::Acknowledge { address }) }
        }
        ...
        _ => {
            if remaining.len() >= (2 + 8 + 6 + 3) && remaining.get(0..2).map_or(false, |s| s.iter().all(|b| b.is_ascii_digit())) {
                ...
                let optional = if remaining.len() > sens_ver_end {
                    ...
                    Some(String::from_utf8(opt_part.to_vec()).map_err(|_| ResponseParseError::InvalidUtf8)?) // ADDED .to_vec()
                } else { None };
            ...
            if remaining.starts_with(b",") && remaining.ends_with(b";") {
                 ...
                 // Ensure ToString trait is in scope via use block above
             ...
             if remaining.starts_with(b"+") || remaining.starts_with(b"-") {
                 ...
                 for i in 1..remaining.len() {
                    if (remaining[i] == b'+' || remaining[i] == b'-') && i > current_start {
                         ...
                         values.push(Sdi12Value::parse_single(value_str)?); // Error mapped via From
                         current_start = i;
                     }
                 }
                 let final_slice = &remaining[current_start..];
                 let final_str = str::from_utf8(final_slice)?;
                 values.push(Sdi12Value::parse_single(final_str)?); // Error mapped via From
        ...
        _ => {
            // If it wasn't Timing, Address, Acknowledge, or Aborted, and alloc is disabled,
            // it must require alloc or be invalid.
...
pub fn parse_binary_packet(buffer: &[u8]) -> Result<Response, ResponseParseError> {
    #[cfg(feature = "alloc")]
    {
        ...
        // Check CRC first, regardless of alloc status
    ...
    #[cfg(not(feature = "alloc"))]
    {
        // Need to check CRC even without alloc
         if buffer.len() < 6 { return Err(ResponseParseError::TooShort); }
          match crc::verify_packet_crc_binary::<()>(buffer) {
             Ok(()) => Err(ResponseParseError::FeatureNotEnabled), // CRC ok, but need alloc for payload
             Err(Sdi12Error::CrcMismatch{..}) => Err(ResponseParseError::CrcMismatch),
             Err(_) => Err(ResponseParseError::InvalidFormat),
         }
    }
}
...
mod tests {
    ...
    #[test] fn test_parse_acknowledge() { /* unchanged */
    ...
    #[test] fn test_parse_aborted() { /* unchanged */
    ...
    #[test] fn test_parse_address_change() { /* unchanged */
        ...
        assert_eq!(parse_response(b"1\r\n"), Ok(Response::Acknowledge { address: addr('1') })); // Correctly acknowledge
    }
    #[test] fn test_parse_timing() { /* Corrected expectations */
         ...
         // Needs alloc feature to differentiate InvalidFormat from FeatureNotEnabled here
         #[cfg(feature = "alloc")]
    ...
    mod alloc_tests {
        ...
        // Import vec! macro and ToString trait
        use alloc::{vec, vec::Vec, string::{String, ToString}};

        #[test] fn test_parse_data_alloc() { /* unchanged, should pass now */
            let resp_a = parse_response(b"0+3.14OqZ\r\n");
            assert!(resp_a.is_ok(), "Ex A failed: {:?}", resp_a);
             if let Ok(Response::Data(info)) = resp_a {
        ...
        #[test] fn test_parse_metadata_alloc() { /* unchanged, should pass now */
            ...
            assert!(resp_b_no_crc.is_ok(), "Meta No CRC failed: {:?}", resp_b_no_crc);
             if let Ok(Response::Metadata(info)) = resp_b_no_crc {
                assert_eq!(info.address, addr('0')); assert_eq!(info.fields, vec!["PR".to_string(), "mm".to_string(), "precipitation rate per day".to_string()]); assert_eq!(info.crc, None);
             } else { panic!("Expected Metadata: {:?}", resp_b_no_crc); }
        }
        #[test] fn test_parse_binary_packet_alloc() { /* unchanged, should pass now */
             ...
             assert!(resp0.is_ok(), "Bin Pkt 0 failed: {:?}", resp0);
              if let Ok(Response::BinaryData(info)) = resp0 {
                 assert_eq!(info.address, addr('1')); assert_eq!(info.packet_size, 4); /* ... */
              } else { panic!("Expected BinaryData: {:?}", resp0); }
        }
        #[test] fn test_parse_binary_packet_errors_alloc() { /* unchanged, should pass now */
            assert!(matches!(parse_binary_packet(b"12345"), Err(ResponseParseError::TooShort)));
            // ... other error cases
        }
        #[test] fn test_parse_response_errors_alloc() { /* unchanged, should pass now */
            assert!(matches!(parse_response(b"0001a\r\n"), Err(ResponseParseError::InvalidFormat)));
            // ... other error cases
        }
    } // end mod alloc_tests
    ...
    #[test]
    fn test_parse_response_errors_no_alloc() { /* Unchanged, check expectations */
        ...
        #[cfg(not(feature = "alloc"))] {
            ...
            // This case might still be InvalidFormat if timing check fails first?
            // Let's trace "0001a\r\n" without alloc:
            // 1. Matches timing length check (len 5)
            // 2. Fails timing `all digits` check.
            // 3. Enters `#[cfg(not(feature = "alloc"))] _ if !remaining.is_empty()` block.
            // 4. Checks if it would have been timing again (returns false).
            // 5. Returns `FeatureNotEnabled`. -> OK
            assert!(matches!(parse_response(b"0001a\r\n"), Err(ResponseParseError::FeatureNotEnabled)));
        }
    }

     #[test]
    fn test_parse_binary_packet_errors_no_alloc() { /* Unchanged, check expectations */
         assert!(matches!(parse_binary_packet(b"12345"), Err(ResponseParseError::TooShort)));
        #[cfg(not(feature = "alloc"))] {
           let packet0_data = &[0x31, 0x04, 0x00, 0x03, 0xFF, 0xFF, 0x01, 0x00];
           let mut packet0_bad_crc = packet0_data.to_vec(); // Needs alloc::vec! macro in scope
           ...
           let mut packet0_good = packet0_data.to_vec(); // Needs alloc::vec!
